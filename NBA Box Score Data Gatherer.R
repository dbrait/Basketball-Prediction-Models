...
#Get any days nba score and box score data
...
get_nba_days_scores <- function(date, return_message=T) {
  ...
  base <- "http://stats.nba.com/stats/scoreboardV2?DayOffset=0&LeagueID=00&gameDate="
  
  url_games <- base %>% paste0(date_stem)
  
  json_data <- url_games %>% fromJSON(simplifyDataFrame = T, flatten = T)
  
  table_names <- json_data$resultSets$name
  
  #Game Header
  if ("GameHeader" %in% table_names) {
    if (json_data$resultSets$rowSet[1] %>% data.frame %>% nrow > 0){
      game_data <- json_data$resultSets$rowSet[1] %>% data.frame %>% tbl_df
      names(game_data) <-
        c("game_date_est",
          "sequence.game",
          "id.game",
      ...
      game_data %<>%
        seperate(gamecode, sep = "\\/", c("date", "teamslugs")) %>%
        mutate(
  ...
  if ("LineScore" %in% table_names) {
    if (json_data$resultSets$rowSet[2] %>% data.frame %>% nrow > 0) {
      ...
      line_score_data %<>%
        seperate(team.record, sep = "\\-", c("wins", "losses")) %>%
  ...
  if ("EastconfStandingsByDay" %in% table_names){
    if (json_data$resultSets$rowSet[5] %>% data.frame %>% nrow > 0) {
      ...
      east_standings_by_day_data %<>%
        ...
        seperate(home_record,
                 sep = "\\-",
                 into = c("wins.home", "losses.home")) %>%
        seperate(road_record,
                 sep = "\\-",
    ...
    east_standings_by_day_data <- data_frame()
  }
  ...
  if (return_message = T) {
    if(team_leader_data %>% nrow > 0){
      fun_data <-
        team_leader_data %>%
        sample_n(1) %>%
        select(name.player.pts_leader, pts, name.taken)
