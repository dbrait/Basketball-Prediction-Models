...
{
  #Much of process is from 
  the.date.file <- fromJSON(file.name)
  #get sports vu data
  ...
  #Add player name information to each movement moment
  home.movements <- merge(home.players, all.movement, by="player_id")
  away.momvements <- merge(away.players, all.movement, by="player_id")
...
player_dist <- function(lastnameA, lastnameB, eventID){
  ...
  df.l <- 1:nrow(dfA)
  distdf <- unlist(lapply(df.l, function(x){dist(rbind(dfA[x,], dfB[x,]))}))
  return(distsdf)
}
...
player_dist_matrix <- function(eventID) {
  ...
  bigdistancedf <- as.data.frame(bigdistancef)
...
#grab a dictionary of gameid's that you can iterate this function over to get all pbp play data
